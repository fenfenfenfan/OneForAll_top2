...
class InsSegEvaluator(Evaluator):
    def __init__(self,
        ...
        if thing_ids is None:
            self.thing_ids = list(range(num_classes))
        else:
    ...
    def update(self, sample_dict, pp_out_dict):
        sem_label = sample_dict['sem_label'].squeeze().numpy()
        preds = self.convert_pred_map(
            pp_out_dict['sem_prob'].squeeze(0).numpy(),
            pp_out_dict['pan_pred'].squeeze().numpy())
        gts = self.convert_gt_map(sem_label,
                                  sample_dict['ins_label'].squeeze().numpy())
    ...
    def evaluate(self):
        ...
        res = {}
        res["AP"] = [{i: ap[i] * 100} for i in self.thing_ids]
        res["mAP"] = 100 * map

        results = build_info_dict(_type_='metric', ins_metrics=res)
        return results
    ...
    def cal_map(self):
        ...
        """
        self.cal_ap()
        valid_ap = [self.ap[i] for i in self.thing_ids]
    ...
    def convert_gt_map(self, seg_map, ins_map):
        """
        Convert the ground truth with format (h*w) to the format that satisfies the AP calculation.
        ...
        for i in instance_cnt:
            if i == 0:
                continue
            mask = ins_map == i
            # Take the first one, since all pixels in the instance belong to the same class
            label = seg_map[mask][0]
            gts.append((label, mask.astype('int64')))
        return gts
    ...
    def convert_pred_map(self, seg_pred, pan_pred):
        ...
        for i in instance_cnt:
            ...
            preds.append((label, score, mask.astype('int64')))
        return preds
