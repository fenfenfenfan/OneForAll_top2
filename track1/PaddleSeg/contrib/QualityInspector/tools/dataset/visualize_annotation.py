...
def parse_args():
    ...
    parser.add_argument(
        '--image_path',
        default='',
        help='images root path, default None if img path in file_path is absolute path',
        type=str)
    parser.add_argument(
...
def get_images_path(file_path, image_root=''):
    """
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            origin_path = os.path.join(image_root, origin_path)
            annot_path = os.path.join(image_root, annot_path)
...
def visualize_imgs(args):
    ...
    images_root = args.image_path
    weight = 0.1

    images_path = get_images_path(file_path, images_root)
