...
def fined_loss_computation(logits_list, losses):
    loss_logit = logits_list[0]
    loss_label = logits_list[1]

    loss_type = losses['types'][0]
    loss_weight = losses['coef'][0]

    return [loss_weight * loss_type(loss_logit, loss_label)]
...
def train(model,
    ...
    """
    coase_params = paddle.load(coase_model_path)
