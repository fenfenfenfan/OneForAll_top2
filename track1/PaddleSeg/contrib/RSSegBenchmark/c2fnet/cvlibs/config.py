...
class Config(object):
    ...
    @property
    def coase_model(self) -> paddle.nn.Layer:
        coase_model_cfg = self.dic.get('coase_model').copy()
        if not self._coase_model:
            self._coase_model = self._load_object(coase_model_cfg)
        return self._coase_model
