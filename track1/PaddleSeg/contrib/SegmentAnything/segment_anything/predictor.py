...
class SamPredictor:
    ...
    def predict(
        ...
        if point_coords is not None:
            ...
            labels_paddle = paddle.to_tensor(point_labels).cast('int32')
            coords_paddle, labels_paddle = coords_paddle[
                None, :, :], labels_paddle[None, :]
        if box is not None:
            box = self.transform.apply_boxes(box, self.original_size)
            box_paddle = paddle.to_tensor(box).cast('float32')
            box_paddle = box_paddle[None, :]
        if mask_input is not None:
            mask_input_paddle = paddle.to_tensor(mask_input).cast('float32')
