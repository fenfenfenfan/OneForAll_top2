...
class ConvMlp(nn.Layer):
    ...
    def __init__(self, in_features, hidden_features, dropout=0.,lr=1.0):
        super().__init__()
        w_attr_1, b_attr_1 = self._init_weights(lr)
    ...
    def _init_weights(self,lr=1.0):
        weight_attr = paddle.ParamAttr(initializer=paddle.nn.initializer.TruncatedNormal(std=.02),learning_rate=lr)
        bias_attr = paddle.ParamAttr(initializer=paddle.nn.initializer.Constant(0.0),learning_rate=lr)
...
class ConvNeXtBlock(nn.Layer):
    def __init__(self,
        ...
        self.conv_dw = nn.Conv2D(dim, dim, kernel_size=7, padding=3, groups=dim,weight_attr=ParamAttr(learning_rate=lr), bias_attr=ParamAttr(learning_rate=lr))  # depthwise conv
        self.norm = norm_layer(dim)
        self.mlp = mlp_layer(dim, int(mlp_ratio * dim),lr=lr)
        self.gamma = paddle.create_parameter(
            shape=[dim], dtype='float32',
            default_initializer=paddle.nn.initializer.Constant(ls_init_value), attr=ParamAttr(learning_rate=lr))
    ...
    def __init__(self,
        ...
        if in_chs != out_chs or stride > 1:
            self.downsample = nn.Sequential(
                norm_layer(in_chs),
                nn.Conv2D(in_chs, out_chs, kernel_size=stride, stride=stride,weight_attr=ParamAttr(learning_rate=lr), bias_attr=ParamAttr(learning_rate=lr)))
        else:
    ...
    def __init__(self,
        ...
        if stem_type == 'patch':
            self.stem = nn.Sequential(
                nn.Conv2D(in_channels, dims[0], kernel_size=patch_size, stride=patch_size,weight_attr=ParamAttr(learning_rate=lr), bias_attr=ParamAttr(learning_rate=lr)),
                norm_layer(dims[0]))
            curr_stride = patch_size
            prev_chs = dims[0]
        else:
            self.stem = nn.Sequential(
                nn.Conv2D(in_channels, 32, kernel_size=3, stride=2, padding=1, weight_attr=ParamAttr(learning_rate=lr), bias_attr=ParamAttr(learning_rate=lr)),
                norm_layer(32),
                nn.GELU(),
                nn.Conv2D(32, 64, kernel_size=3, padding=1,weight_attr=ParamAttr(learning_rate=lr), bias_attr=ParamAttr(learning_rate=lr)))
            curr_stride = 2
            prev_chs = 64
        ...
        for i in range(4):
            ...
            stages.append(ConvNeXtStage(
                ...
                norm_layer=norm_layer, cl_norm_layer=cl_norm_layer, lr=lr)
            )
            prev_chs = out_chs
