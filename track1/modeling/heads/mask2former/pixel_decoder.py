...
from .param_init import c2_xavier_fill, THLinearInitMixin
...
class MSDeformAttnPixelDecoder(nn.Layer):
    def __init__(self, num_heads, ff_dim, num_enc_layers, conv_dim, mask_dim,
                 in_feat_strides, in_feat_chns, feat_indices, common_stride, maskformer_num_feature_levels, use_neck):
        super().__init__()
        ...
        if not self.use_neck:
            print('==================>use default seg head neck')
            input_proj_list = []
    ...
    def forward(self, features):
        ...
        for idx, ti in enumerate(self.transformer_indices[::-1]):
            ...
            if not self.use_neck:
                srcs.append(self.input_proj[idx](x))  #backbone特征图尺寸从小到大排列
            else:
                srcs.append(x)
