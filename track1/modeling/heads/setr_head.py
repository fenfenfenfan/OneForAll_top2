...
class PUPHead(nn.Layer):
    ...
    def forward(self, x, _shape):
        logits = []
        feat = x[self.backbone_indices[-1]]
        # print(feat.shape,_shape[2],_shape[3],self.in_channels)
        # print(feat.reshape([2,self.in_channels, _shape[2]*_shape[3]//4]).shape)
        feat = feat.reshape([0,self.in_channels, _shape[2]*_shape[3]//4]).transpose([0, 2, 1])
        # print(feat.shape) # [2, 560, 768]
        feat = self.cls_head_norm(feat).transpose([0, 2, 1]).reshape(
            [0, self.in_channels, _shape[2]//2, _shape[3]//2])
        # print(feat.shape) # [2, 768, 40, 14]
        logits.append(self.cls_head(feat))
        ...
        if self.training:
            for idx, _head in enumerate(self.aux_heads):
                ...
                feat = self.aux_head_norms[idx](feat).transpose(
                    [0, 2,
                     1]).reshape([0, self.in_channels, _shape[2]//2, _shape[3]//2])
                logits.append(_head(feat))
