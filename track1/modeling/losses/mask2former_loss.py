...
class Mask2FormerLoss(nn.Layer):
    def __init__(self, 
                 num_classes=150,
                 eos_coef=0.1,
                 loss_ce = 2.0,
                 mask_weight = 5.0,
                 dice_weight = 5.0, #损失权重
                 cost_loss_ce = 2.0,  #计算cost权重
        ...
        self.losses_content = losses

        self.loss1 = mask2Formerloss(
                            loss_ce = loss_ce,
                            mask_weight = mask_weight,
                            dice_weight = dice_weight,
                            cost_loss_ce = cost_loss_ce,
                            cost_mask_weight = cost_mask_weight,
                            cost_dice_weight = cost_dice_weight,
                            num_classes = self.num_classes,
                            eos_coef=self.eos_coef,
                            losses=self.losses_content,
                            )


        self.losses = [self.loss1]
        self.seg_loss_weight = seg_loss_weight
        self.coef = [1.0]

    def forward(self,outputs, targets):
        loss_list = self.loss_computation(outputs, targets, self.losses, self.coef)
        loss = dict()
        loss['seg_setr_loss'] = sum(loss_list)  * self.seg_loss_weight
        return loss
    ...
    def loss_computation(self, logits_list, targets, losses, coef):
...
class mask2Formerloss(nn.Layer):
    ...
    def __init__(self,
        ...
        self.cost_loss_ce = cost_loss_ce
        ...
        self.matcher = HungarianMatcher(
            cost_class=self.cost_loss_ce, cost_mask=self.cost_mask_weight, cost_dice=self.cost_dice_weight)
        self.losses = losses
        ...
        self.importance_sample_ratio = 0.7
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        target_masks = target_masks[tgt_idx]  #gt mask
    ...
    def forward(self, logits, targets):
        ...
        for target_per_image_idx in range(batch_size):
            ...
            targets_cpt.append({  
                ...
                "masks": padded_masks,
                "image_path":  targets['image_path'][target_per_image_idx]
            })
        # delete ignore ind
        targets = []
