...
class MultiTaskBatchFuse(nn.Layer):
    ...
    def forward(self, task_batched_inputs):
        ...
        for task_name, batched_inputs in task_batched_inputs.items():
            ...
            # ['segmentation', 'fgvc', 'trafficsign']
            if self.multi_branch=='one':
                features = self.backbone(self.preprocess_image(batched_inputs))
            elif self.multi_branch=='multi':
                if task_name=='fgvc':
                    flag='cls'
                    ...
                    flag='det'
                features = self.backbone(self.preprocess_image(batched_inputs),flag)
            else:
                print('error input type, choose one type backbone, one or multi')
